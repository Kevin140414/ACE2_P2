...
public class ConexionBT {
    ...
    private synchronized void setState(int estado) {
        ...
        mHandler.obtainMessage(TakeShowerWindow.Mensaje_Estado_Cambiado, estado, -1).sendToTarget();
    }
    ...
    public synchronized void connected(BluetoothSocket socket, BluetoothDevice device) {
        ...
        Message msg = mHandler.obtainMessage(TakeShowerWindow.Mensaje_Nombre_Dispositivo);
        Bundle bundle = new Bundle();
        bundle.putString(TakeShowerWindow.DEVICE_NAME, device.getName());
        msg.setData(bundle);
    ...
    private void connectionFailed() {
        ...
        Message msg = mHandler.obtainMessage(TakeShowerWindow.Mensaje_TOAST);
        Bundle bundle = new Bundle();
        bundle.putString(TakeShowerWindow.TOAST, "Error de conexi?n");
        msg.setData(bundle);
        mHandler.sendMessage(msg);
    }
    ...
    private void connectionLost() {
        ...
        Message msg = mHandler.obtainMessage(TakeShowerWindow.Mensaje_TOAST);
        Bundle bundle = new Bundle();
        bundle.putString( TakeShowerWindow.TOAST, "Se perdio conexion");
        msg.setData(bundle);
        mHandler.sendMessage(msg);
        msg = mHandler.obtainMessage(TakeShowerWindow.MESSAGE_Desconectado);
        mHandler.sendMessage(msg);
...
//<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>
        ...
        public void write(byte[] buffer) {
            try {
                ...
                mHandler.obtainMessage(TakeShowerWindow.Mensaje_Escrito, -1, -1, buffer).sendToTarget();
            } catch (IOException e) {
        ...
        public void run() {
            ...
            while (true) { //Mantiene escuchando el InputStream mientras este conectado
                try {
                    ...
                    mHandler.obtainMessage( TakeShowerWindow.Mensaje_Leido, bytes, -1, buffer).sendToTarget(); //readMessageX por buffer
                } catch (IOException e) {
